...
void main() {
  ...
  // Mouse interaction: distort wind based on cursor distance
  vec2 mouseVec = (gl_FragCoord.xy - uMouse); // Vector from mouse to pixel
  float mouseDist = length(mouseVec);
  float interactionRadius = 90.0; // Reduced radius (User requested smaller)
  float interactionStrength = 8.0; // Stronger push for "deflection" feel
  
  // Falloff: strong near center, zero at radius
  float influence = smoothstep(interactionRadius, 0.0, mouseDist) * interactionStrength;
  vec2 windDistortion = normalize(mouseVec) * influence;
  
  float windX = cos(uDirection) * 0.4 + windDistortion.x;
  float windY = sin(uDirection) * 0.4 - windDistortion.y;
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        if (dist < flakeSize) {
          ...
          // Apply cursor clearing mask
          // Strict clearing: 0% opacity until 60% of radius, then fade in
          float clearMask = smoothstep(interactionRadius * 0.6, interactionRadius, mouseDist);
          intensity *= clearMask;
...
export default function PixelSnow({
  ...
  const mouseRef = useRef(new Vector2(-1000, -1000)); // Initialize off-screen
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      // Update mouse position (Y is inverted in WebGL relative to DOM)
      // Optimization: Store in ref, update uniform in frame loop
